...
const demoBoxes: Detection[] = [
  { id: '01', label: 'Huevo', confidence: .98, box: [.15, .23, .17, .43] },
  { id: '02', label: 'Huevo', confidence: .96, box: [.41, .30, .17, .43] },
  { id: '03', label: 'Huevo', confidence: .93, box: [.67, .22, .17, .43] },
];
export default function Home() {
  ...
  return <div className="shell">
    ...
    <div className="section-label separated"><span>02</span> Conexión al modelo</div><label htmlFor="endpoint">Endpoint del backend</label><input id="endpoint" type="url" placeholder="wss://tu-backend.com/video" value={endpoint} disabled={status !== 'idle'} onChange={e => setEndpoint(e.target.value)} spellCheck={false}/><p className="field-hint">Video continuo por WebSocket. Integración pendiente de definir con tu backend.</p><button className={status === 'idle' ? 'primary full' : 'secondary full'} disabled={status === 'idle' && (!camera || !endpoint.trim())} onClick={status === 'idle' ? startTransmission : stopTransmission}>{status === 'connecting' ? 'Cancelar conexión' : status === 'live' ? 'Detener transmisión' : '⇄  Iniciar transmisión'}</button>
